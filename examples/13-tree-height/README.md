...
| [`tree-height-spec.k`](tree-height-spec.k) | the K reachability claims (contract + circularities) |
...
- [`tree-height-spec.k`](tree-height-spec.k) — program-specific K claims for `tree height`; [`mini-python.k`](mini-python.k) is only the mini-Python semantics.
