# Reproducibility — the prompts that produce `average`
...
```text
[P1]  Write `average(nums)` returning the arithmetic mean of a non-empty list of numbers. Self-contained: sum with your own `while` loop and divide by `len` — no `sum()` or other builtins.
