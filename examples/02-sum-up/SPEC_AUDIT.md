...
`INTENT_SPEC.md` → [`sum-up-spec.k`](sum-up-spec.k) K claims →
...
|---|---|---|
| Prompt obligation represented | PASS | P1 obligation is recorded in [`INTENT_SPEC.md`](INTENT_SPEC.md) and encoded by the claim provenance in [`sum-up-spec.k`](sum-up-spec.k). |
| Program-specific claim naming | PASS | Claims live in [`sum-up-spec.k`](sum-up-spec.k); [`mini-python.k`](mini-python.k) is only the mini-Python semantics. |
