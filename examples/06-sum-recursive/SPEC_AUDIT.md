# Spec adequacy audit — `sum_recursive.py`

This file performs the adequacy round-trip required by the current FVK protocol:

`INTENT_SPEC.md` → [`sum-recursive-spec.k`](sum-recursive-spec.k) K claims →
...
|---|---|---|
| Prompt obligation represented | PASS | P1 obligation is recorded in [`INTENT_SPEC.md`](INTENT_SPEC.md) and encoded by the claim provenance in [`sum-recursive-spec.k`](sum-recursive-spec.k). |
| Program-specific claim naming | PASS | Claims live in [`sum-recursive-spec.k`](sum-recursive-spec.k); [`mini-python.k`](mini-python.k) is only the mini-Python semantics. |
