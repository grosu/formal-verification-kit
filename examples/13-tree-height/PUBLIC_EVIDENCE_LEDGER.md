# Public evidence ledger — `tree_height.py`
...
[`tree-height-spec.k`](tree-height-spec.k).

| ID | Source | Evidence | Obligation / use | Status |
|---|---|---|---|---|
| P1 | prompt | `Write `tree_height(t)` for a binary tree (`None`, or a `(value, left, right)` tuple), recursively. Use your own `max2` helper (no `max`). Self-contained.` | `tree_height(t)` should return the height of a finite well-formed binary tree; helper `max2` should return the larger of two heights. | Intent source for the function contract(s). |
| API | public symbol/name | `max2`, `tree_height` in [`tree_height.py`](tree_height.py) | The named public function(s) are the verified unit(s). | Encoded by the program-specific claim file. |
