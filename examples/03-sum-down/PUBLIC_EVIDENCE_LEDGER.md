...
[`sum-down-spec.k`](sum-down-spec.k).

| ID | Source | Evidence | Obligation / use | Status |
|---|---|---|---|---|
| P1 | prompt | `Let's write a very simple Python program which calculates the sum of numbers from 1 to n. Let's call it sum. It takes an input integer n. Let's do it using a while loop iterating from n all the way down to 1.` | `sum_to_n(n)` should compute `1 + ... + n` using a decreasing counter on the intended non-negative domain. | Intent source for the function contract(s). |
