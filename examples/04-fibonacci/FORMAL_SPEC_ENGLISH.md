# Formal spec in English — `fib.py`

This file is the English paraphrase of the program-specific K claims in
[`fibonacci-spec.k`](fibonacci-spec.k). The language-fragment semantics live separately in
[`mini-python.k`](mini-python.k); the `*-spec.k` file names the **program** being
specified, not the language.

## Claims / circularities

- **C1.** (LOOP) coupled-accumulator loop circularity relating `prev`,`curr` to consecutive Fibonacci values.
- **C2.** (FIB) function contract `result = fib(N)` for `N >= 0`.

## Overall contract shape

The claims formalize the public obligation from [`INTENT_SPEC.md`](INTENT_SPEC.md):

> `fib(n)` should return the nth Fibonacci number for `n >= 0`, with `fib(0)=0` and `fib(1)=1`.
