# Formal spec in English — `insertion_sort.py`

This file is the English paraphrase of the program-specific K claims in
[`insertion-sort-spec.k`](insertion-sort-spec.k). The language-fragment semantics live separately in
[`mini-python.k`](mini-python.k); the `*-spec.k` file names the **program** being
specified, not the language.

## Claims / circularities

- **C1.** (INNER) inner shift-loop circularity.
- **C2.** (OUTER) outer sorted-prefix circularity.
- **C3.** (SORT) function contract for in-place sorted permutation.

## Overall contract shape

The claims formalize the public obligation from [`INTENT_SPEC.md`](INTENT_SPEC.md):

> `insertion_sort(a)` should sort the same list in place, returning the mutated list as a sorted permutation of the input.
