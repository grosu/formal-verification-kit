# Formal spec in English — `binary_search.py`

This file is the English paraphrase of the program-specific K claims in
[`binary-search-spec.k`](binary-search-spec.k). The language-fragment semantics live separately in
[`mini-python.k`](mini-python.k); the `*-spec.k` file names the **program** being
specified, not the language.

## Claims / circularities

- **C1.** (LOOP) narrowing-window loop circularity for binary search.
- **C2.** (SEARCH) function contract for found/absent behavior under sortedness.

## Overall contract shape

The claims formalize the public obligation from [`INTENT_SPEC.md`](INTENT_SPEC.md):

> `binary_search(a,x)` should return some index whose value is `x` when present in a sorted, totally ordered list, and `-1` when absent.
