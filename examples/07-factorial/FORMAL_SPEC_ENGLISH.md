# Formal spec in English — `factorial.py`

This file is the English paraphrase of the program-specific K claims in
[`factorial-spec.k`](factorial-spec.k). The language-fragment semantics live separately in
[`mini-python.k`](mini-python.k); the `*-spec.k` file names the **program** being
specified, not the language.

## Claims / circularities

- **C1.** (REC) recursive-call circularity for `factorial(N)`.
- **C2.** (FACT) user-facing function contract `result = fact(N)` for `N >= 0`.

## Overall contract shape

The claims formalize the public obligation from [`INTENT_SPEC.md`](INTENT_SPEC.md):

> `factorial(n)` should return `n!` for non-negative integers, with `0! = 1`.
