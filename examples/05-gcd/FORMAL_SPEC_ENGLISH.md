# Formal spec in English — `gcd.py`

This file is the English paraphrase of the program-specific K claims in
[`gcd-spec.k`](gcd-spec.k). The language-fragment semantics live separately in
[`mini-python.k`](mini-python.k); the `*-spec.k` file names the **program** being
specified, not the language.

## Claims / circularities

- **C1.** (LOOP) Euclidean preserved-relation loop circularity.
- **C2.** (GCD) function contract `result = gcd(A,B)` for `A,B >= 0`.

## Overall contract shape

The claims formalize the public obligation from [`INTENT_SPEC.md`](INTENT_SPEC.md):

> `gcd(a,b)` should return the non-negative greatest common divisor of two non-negative integers.
